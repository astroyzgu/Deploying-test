...
def collect4csstmock(snapnum, basedir_hbt, basedir_groups):
    ...
    >>> 3. 'host_id': host halo ID in present snapshot
    ...
    >>> 9.   'sub_accmass': subhalo mass at accretion
...
#
    ...
    end_time = time.time()
    print('reading %sth snapshot time is %.3f s' %(snapnum-1, end_time - start_time))
...
#
    ...
    INPUT_FORT_LONG[:,0] = DATACOLLECT_hbt['TrackId'][:]
    INPUT_FORT_LONG[:,1] = DATACOLLECT_hbt['Rank'][:]
    INPUT_FORT_LONG[:,2] = DATACOLLECT_groups['group_nr'][idx_host]
    ...
    INPUT_FORT_LONG[:,3] = nexthaloid(  DATACOLLECT_hbt['TrackId'], \
                                        DATACOLLECT_hbt_next['TrackId'], \
                                        next_groups_id )
    ...
    INPUT_FORT_DOUBLE[:,6]   = DATACOLLECT_groups['group_mass'][idx_host]
    ...
    end_time = time.time()
    print('reading %sth snapshot time is %.3f s' %(snapnum, end_time - start_time) )
...
def lightcone4csstmock():
    ...
    >>> INPUT_FORT_LONG (np.int64):
    >>> 1.  ?????: halo ID from fof, 'group_nr' 
    >>> 2. 'TrackID': subhalo id
    >>> 3. 'rank': rank==1: central halo (most massive); rank ==0: subhalos
    
    >>> INPUT_FORT_DOUBLE (np.float64): 
    >>> 1.   'ra': Right Ascension
    >>> 2.   'dec': Declination 
    >>> 3.   'd_comoving': comoving distance
    >>> 3.   'redshift_true': cosmology redshift, z_cos
    >>> 4.   'redshift_obs': observed redshift, z_obs 
    >>> 5.   'v_lfs': line-of-sight velocity 
    >>> 7.   'Mvir':  host halo mass
...
#              'shMbound', 'd_comoving', 'ra', 'dec', 'vmax', 'PeakMass', 'PeakVmax', 'Mvir','Rvir',
#              'shBoundM200Crit','M200mean', 'redshift_true', 'redshift_obs', 'shMbound_at_ac', 'snapNum']
    props = ['trackID', 'hosthaloID', 'rank', \
             'ra', 'dec', 'd_comoving', \
             'redshift_true', 'redshift_obs', 'v_lfs', \
             'shMbound', 'PeakMass', 'vmax'] 
    ret = read_edition0(props, save=False)
    INPUT_FORT_LONG    = np.array(ret[  ['trackID', 'hosthaloID']  ]).astype('int64')
    INPUT_FORT_DOUBLE  = np.array(ret[  ['ra', 'dec', 'd_comoving', \
             'redshift_true', 'redshift_obs', 'v_lfs', \
             'Mvir', 'shMbound', 'PeakMass', 'vmax']  ]).astype('float64')
    Nsubs = INPUT_FORT_LONG.shape[0]
    return Nsubs, INPUT_FORT_LONG,  INPUT_FORT_DOUBLE

if __name__ == '__main__0': 
    Nsubs, INPUT_FORT_LONG,  INPUT_FORT_DOUBLE = lightcone4csstmock() 
    print(Nsubs) 
    #print(ret['trackID'][:10])
